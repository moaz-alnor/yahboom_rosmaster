 ...
 * 
 * This program implements a ROS 2 action server that eecutes timed rotation actions for the Yahboom RosMaster robot.
 * It recieves goals from action clients, publishes timestamped velocity commands to rotate the robot,
 ...
 */
  ...
  * @brief ROS @ action server for the TimedRotation action.
  * 
  * This class implements the action server functionality for the TimedRotatin action.
  * It recieves goals from an action client, executes the timed rotation, provides feedback,
  * and handles goal cancellation requests. The server maintains rotation by continuously
  * publishing timestamped velocity commands until the specified duration is reached or the 
  * goal is canceled.
  */ 
